...
//        baseView.showDialogLoading();
        call.enqueue(new Callback<ResponseBody>() {
            ...
            public void onResponse(Call<ResponseBody> call, Response<ResponseBody> response) {
                try {
                    ...
                    UploadImageBean uploadImageBean = (UploadImageBean) new Gson().fromJson(jsonString,
    ...
    public synchronized void requestDateNoLog(Observable observable, final String isDialog,
        ...
        mSubscription = observable.observeOn(AndroidSchedulers.mainThread()).subscribe(new Subscriber() {
            @Override
            public void onStart() {
                super.onStart();
    ...
    private void deleteUselessIp() {
        ...
        if (null != ipBeanList && ipBeanList.size() > 0) {
            for (IpBean ipBean : ipBeanList) {
                String baseUrl =
                        ipBean.getSchema() + "://" + ipBean.getHost() + ":" + ipBean.getPort() + "/" + ipBean.getContent()+"/";
                if (RetrofitManager.getInstance().getBaseUrl().equals(baseUrl)) {
                    ipBeanList.remove(ipBean);
                    SPUtils.putList(Constants.SP_IPLIST_OBJECT,ipBeanList);
                    break;
                }
            }
        }
    ...
    public Subscription requestSubscript(Observable observable, final String isDialog, final BaseCallBack callBack) {
        ...
        Subscription subscription = observable.observeOn(AndroidSchedulers.mainThread()).subscribe(new Subscriber() {
            @Override
            public void onStart() {
                super.onStart();
                if (baseView != null) {
    ...
    public Subscription requestNoLogSubscript(Observable observable, final String isDialog,
            final BaseCallBack callBack) {
        baseView = (BaseView) getView();
        if (!NetWorkUtils.isNetworkConnected(BaseApplication.getInstance().getContext())) {
            if (isDialog != null && baseView != null) {
