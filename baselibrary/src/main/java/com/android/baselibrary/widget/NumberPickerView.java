...
public class NumberPickerView extends View {
    ...
    private static final int DEFAULT_DIVIDER_COLOR = 0XFFFFD839;
    ...
    private void initAttr(Context context, AttributeSet attrs) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            } else if (attr == R.styleable.NumberPickerView_npv_marginLeft) {
                viewMarginLeft = a.getDimensionPixelOffset(attr, 0);
            }
        }
        a.recycle();
    }
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        mViewCenterX = ((float) (mViewWidth + getPaddingLeft() - getPaddingRight())) / 2 + viewMarginLeft;
        int defaultValue = 0;
    ...
    private void updateDividerAttr() {
        ...
        dividerY0 = mDividerIndex0 * mViewHeight / mShowCount - 10;
        dividerY1 = mDividerIndex1 * mViewHeight / mShowCount + 10;
    ...
    private void drawContent(Canvas canvas) {
        ...
        for (int i = 0; i < mShowCount + 1; i++) {
            ...
            if (i == mShowCount / 2) {            //this will be picked
                ...
                fraction = 0f;
            } else if (i == mShowCount / 2 + 1) {  //当前选择的数字 下数第一个
                textColor = getEvaluateColor(0.5f - fraction, mTextColorNormal, mTextColorSelected);
                textSize = getEvaluateSize(0.5f - fraction, mTextSizeNormal, mTextSizeSelected);
                textSizeCenterYOffset = getEvaluateSize(1f - fraction, mTextSizeNormalCenterYOffset,
                        mTextSizeSelectedCenterYOffset) + 10;
            } else if (i == mShowCount / 2 + 2) {  //当前选择的数字 下数第二个
                textColor = getEvaluateColor(0f - fraction, mTextColorNormal, mTextColorSelected);
                textSize = getEvaluateSize(0f - fraction, mTextSizeNormal, mTextSizeSelected);
                textSizeCenterYOffset = getEvaluateSize(1f - fraction, mTextSizeNormalCenterYOffset,
                        mTextSizeSelectedCenterYOffset) + 6;
            } else if (i == Math.floor(mShowCount / 2 - 1)) {  //当前选择的数字 上数第一个
                textColor = getEvaluateColor(0.5f - fraction, mTextColorNormal, mTextColorSelected);
                textSize = getEvaluateSize(0.5f - fraction, mTextSizeNormal, mTextSizeSelected);
                textSizeCenterYOffset = getEvaluateSize(1f - fraction, mTextSizeNormalCenterYOffset,
                        mTextSizeSelectedCenterYOffset) - 10;
            } else if (i == Math.floor(mShowCount / 2 - 2)) {  //当前选择的数字 上数第二个
                textColor = getEvaluateColor(0f - fraction, mTextColorNormal, mTextColorSelected);
                textSize = getEvaluateSize(0f - fraction, mTextSizeNormal, mTextSizeSelected);
                textSizeCenterYOffset = getEvaluateSize(1f - fraction, mTextSizeNormalCenterYOffset,
                        mTextSizeSelectedCenterYOffset) - 6;
    ...
    private void drawHint(Canvas canvas) {
        ...
        canvas.drawText(mHintText,
                mViewCenterX - (mMaxWidthOfDisplayedValues + mWidthOfHintText) / 2 - mMarginEndOfHint,
