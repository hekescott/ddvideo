...
public class GlideUtils {
    ...
    public void LoadContextRoundAndCeterCropBitmapDefault(Context context, String path, ImageView imageView, int placeid, int errorid, String bitmapOrgif, int roundradius) {
        WeakReference<Context> w_context = new WeakReference<Context>(context);
        Context weakContext = w_context.get();
        if (roundradius <= 0) {
            Glide.with(weakContext).load(path)
                    .bitmapTransform(new GlideRoundTransform(context)).placeholder(placeid).error(errorid).crossFade().into(imageView);
        } else {
            Glide.with(weakContext).load(path)
                    .bitmapTransform(new CenterCrop(context), new GlideRoundTransform(context, roundradius)).placeholder(placeid).error(errorid).crossFade().into(imageView);
        }
    }
     ...
     */
    public void LoadContextBitmap(Context context, String path, final ImageView imageView, int placeid, int errorid, String bitmapOrgif) {
//        Glide.with(context)
...
//                        super.setResource(resource);
    ...
    public void LoadContextBitmapOverrride(Context context, String path, final ImageView imageView, int placeid, int errorid, String bitmapOrgif) {
//        Glide.with(context)
//                .load(path + MNetStatusHandle.QUALITY)
//                .placeholder(placeid).error(errorid)
//                .skipMemoryCache(true)
//                .crossFade().into(imageView);
        if (context != null && Util.isOnMainThread()) {
            Glide.with(context.getApplicationContext())
                    .load(path).asBitmap()
                    .placeholder(placeid).error(errorid)
                    .transform(new FitCenter(context),new GlideRoundTransform(context, 6))
                    .crossFade()
                    .override((ScreenUtils.getScreenWidth(context)- ScreenUtil.dip2px(context,20)), Integer.MAX_VALUE)//给个最大值
                    .skipMemoryCache(true)
                    .into(new BitmapImageViewTarget(imageView){
                        @Override
                        protected void setResource(Bitmap resource) {
//                        super.setResource(resource);
    ...
    public void LoadNewContextBitmap(Context context, String path, ImageView imageView, int placeid, int errorid, String bitmapOrgif) {
//        Glide.with(context)
//                .load(path + MNetStatusHandle.QUALITY)
//                .placeholder(placeid).error(errorid)
//                .skipMemoryCache(true)
//                .crossFade().into(imageView);
    ...
    public void LoadContextBitmapGif(Context context, @DrawableRes int drawableId, ImageView imageView, int placeid, int errorid) {
        Glide.with(context)
                .load(drawableId)
                .asGif()
                .skipMemoryCache(true)
    ...
    public void LoadContextBitmap(Context context, @DrawableRes int drawableId, ImageView imageView, int placeid, int errorid) {
        Glide.with(context)
                .load(drawableId)
                .dontAnimate().centerCrop()
                .placeholder(placeid).error(errorid)
    ...
    public void LoadContextBitmap2(Context context, String path, ImageView imageView, int placeid, int errorid, String bitmapOrgif) {
        if (bitmapOrgif == null || bitmapOrgif.equals(LOAD_BITMAP)) {
            Glide.with(context).load(path + "@1500w_1500h_100Q").placeholder(placeid).error(errorid).crossFade().dontAnimate().into(imageView);
        } else if (bitmapOrgif.equals(LOAD_GIF)) {
            Glide.with(context).load(path + "@1500w_1500h_100Q").asGif().crossFade().dontAnimate().into(imageView);
        }
    }
     ...
     * @param bitmapOrgif 加载普通图片 或者GIF图片 ，GIF图片设置bitmap显示第一帧
     */
    public void LoadSupportv4FragmentBitmap(Fragment fragment, String path
            , ImageView imageView, int placeid, int errorid, String bitmapOrgif) {
        if (bitmapOrgif == null || bitmapOrgif.equals(LOAD_BITMAP)) {
            Glide.with(fragment.getContext()).load(path).placeholder(placeid).error(errorid).crossFade()
                    .into(imageView);
        } else if (bitmapOrgif.equals(LOAD_GIF)) {
            Glide.with(fragment.getContext()).load(path).asGif().crossFade().into(imageView);
        }
    }
    //---------------------圆形图片-------------------
    ...
    public void LoadContextCircleBitmap(Context context, String path, ImageView imageView, int placeid, int errorid, String bitmapOrgif) {
        ...
        if (bitmapOrgif == null || bitmapOrgif.equals(LOAD_BITMAP)) {
            Glide.with(context).load(path).placeholder(placeid).error(errorid).crossFade()
...
//                .placeholder(placeid)
//                .into(imageView);
        Glide.with(context).load(path).asBitmap().centerCrop().placeholder(placeid).error(errorid).into(new BitmapImageViewTarget(imageView) {
            @Override
            protected void setResource(Bitmap resource) {
    ...
    public void LoadfragmentCircleBitmap(android.app.Fragment fragment, String path, ImageView imageView, int placeid, int errorid, String bitmapOrgif) {
        ...
        if (bitmapOrgif == null || bitmapOrgif.equals(LOAD_BITMAP)) {
            Glide.with(fragment).load(path).placeholder(placeid).error(errorid).crossFade()
     ...
     */
    @SuppressWarnings("unchecked")
    public void LoadSupportv4FragmentCircleBitmap(Fragment fragment, String path, ImageView imageView, int placeid, int errorid, String bitmapOrgif) {
        //Glide.with(fragment).load(path).bitmapTransform(new GlideCircleTransform(fragment.getActivity())).into(imageView);
        if (bitmapOrgif == null || bitmapOrgif.equals(LOAD_BITMAP)) {
            Glide.with(fragment.getContext()).load(path).placeholder(placeid).error(errorid).crossFade()
                    .bitmapTransform(new GlideCircleTransform(fragment.getActivity())).into(imageView);
    ...
    public void LoadContextRoundBitmapDefault(Context context, String path, ImageView imageView, int placeid, int errorid, String bitmapOrgif, int roundradius) {
        if (roundradius <= 0) {
            Glide.with(context).load(path).bitmapTransform(new GlideRoundTransform(context)).placeholder(placeid).error(errorid).crossFade().into(imageView);
        } else {
            Glide.with(context).load(path).bitmapTransform(new GlideRoundTransform(context, roundradius)).placeholder(placeid).error(errorid).crossFade().into(imageView);
        }
    }
    ...
    public void LoadSupportv4FragmentRoundBitmap(Fragment fragment, String path, ImageView imageView, int roundradius) {
        if (roundradius < 0) {
            Glide.with(fragment.getContext()).load(path).bitmapTransform(new GlideRoundTransform(fragment.getActivity())).into(imageView);
        } else {
            Glide.with(fragment.getContext()).load(path).bitmapTransform(new GlideRoundTransform(fragment.getActivity(), roundradius)).into(imageView);
        }
    }
    //-------------------------------------------------
    ...
    public void LoadSupportv4FragmentRotateBitmap(Fragment fragment, String path, ImageView imageView, Float rotateRotationAngle) {
        Glide.with(fragment.getContext()).load(path).bitmapTransform(new RotateTransformation(fragment.getActivity(), rotateRotationAngle)).into(imageView);
    }
    //----------------------旋转---------------------------
