...
public interface NetApi {
     ...
     */
    @Headers({"Content-type:application/json;charset=UTF-8"})
    @POST(" openapi/allTags")
    Observable<DefaultBean<AllTagsDataBean>> allTags(@Body RequestBody route);
    /**
     * 获取所有专题标签
     */
    @Headers({"Content-type:application/json;charset=UTF-8"})
    @POST("/openapi/getBackupIps")
    Observable<DefaultBean<IpListBean>> getBackupIps(@Body RequestBody route);

    /**
     * 获取推广信息次数
     */
    @Headers({"Content-type:application/json;charset=UTF-8"})
    @POST("openapi/tuiguang")
    Observable<PromoteNumBean> getPromoteNum(@Body RequestBody route);
    ...
    Observable<NotificationDetailBean> getNoticeDetail(@Body RequestBody route);
}
