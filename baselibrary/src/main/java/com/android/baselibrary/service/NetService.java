...
public class NetService {
     ...
     * 获取所有专题标签
     */
    public Observable<DefaultBean<AllTagsDataBean>> allTags() {
        HashMap<String,String> paramsMap = new HashMap<>();
        RequestBody body = this.requestHelper.getHttpRequestMap(paramsMap);
        return netApi.allTags(body).subscribeOn(Schedulers.io());
    }

    /**
     * 获取IP
     */
    public Observable<DefaultBean<IpListBean>> getBackupIps() {
        HashMap<String, String> paramsMap = new HashMap<>();
        RequestBody body = this.requestHelper.getHttpRequestMap(paramsMap);
        return netApi.getBackupIps(body).subscribeOn(Schedulers.io());
    }

    /**
     * 获取推广信息次数
     */
    public Observable<PromoteNumBean> getPromoteNum() {
        HashMap<String, String> paramsMap = new HashMap<>();
        RequestBody body = this.requestHelper.getHttpRequestMap(paramsMap);
        return netApi.getPromoteNum(body).subscribeOn(Schedulers.io());
    }

    /**
     * 获取公告详情
     */
    public Observable<NotificationDetailBean> getNoticeDetail(String id) {
        HashMap<String, String> paramsMap = new HashMap<>();
        paramsMap.put("id", id);
        RequestBody body = this.requestHelper.getHttpRequestMap(paramsMap);
        return netApi.getNoticeDetail(body).subscribeOn(Schedulers.io());
    }
}