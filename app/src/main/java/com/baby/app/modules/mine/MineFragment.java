...
import com.baby.app.modules.mine.page.RechargeActivity;
...
public class MineFragment extends IBaseFragment implements View.OnClickListener, MineView {
    ...
    public void initUiAndListener(View view) {
        ...
        mineSettintBtn = view.findViewById(R.id.mine_setting_btn);
        mineSettintBtn.setOnClickListener(this);

        rv_main_list = view.findViewById(R.id.m_rv_home_list);
        rl_main_list = view.findViewById(R.id.m_rl_home_list);
        initAdapter();
...
//                openActivity(CommonWebViewActivity.class, bundle);
                startActivity(new Intent(Intent.ACTION_VIEW, Uri.parse(UserStorage.getInstance().getQqurl())));
            }
...
//                bundle.putString("url", UrlConstants.FEED_BAKK_URL);
                startActivity(new Intent(Intent.ACTION_VIEW, Uri.parse(UrlConstants.FEED_BAKK_URL)));
//                openActivity(CommonWebViewActivity.class, bundle);
            }
            ...
            public void gotoCacheDetail(DownLoadInfo downLoadInfo) {
                jumpToVideo2(Integer.parseInt(downLoadInfo.getVideoId()), downLoadInfo.getName(),
    ...
    public void refreshMineData(boolean isRefresh) {
        if (UserStorage.getInstance().getBottomAdv().size() > 0) {//添加广告
            if (mMineTypeAdapter.getData().get(2).getItemType() != MineTypeBean.LAYOUT_MINE_5) {
                mMineTypeAdapter.addAdv();
                mMineTypeAdapter.addData(2, new MineTypeBean(MineTypeBean.LAYOUT_MINE_5));
            }
        }
        mMineTypeAdapter.notifyDataSetChanged();
