...
import com.baby.app.util.JumpToUtil;
...
//                    break;
                case MineTypeBean.LAYOUT_MINE_2:
                    ...
                    break;
                case MineTypeBean.LAYOUT_MINE_5:
                    addItemType(MineTypeBean.LAYOUT_MINE_5, R.layout.item_mine_bottom_adv);
                    break;
...
//            break;
            case MineTypeBean.LAYOUT_MINE_2: {
                ...
                //TODO:推广
                GlideUtils.getInstance().LoadContextBitmap(
                        mContext,
                        R.mipmap.ic_my_promote,
                        (ImageView)helper.getView(R.id.myPromoteImg),
                        R.mipmap.ic_my_promote,
                        R.mipmap.ic_my_promote);
...
//                        }
            ...
            case MineTypeBean.LAYOUT_MINE_5://广告
                if (UserStorage.getInstance().getBottomAdv().size() > 0) {
                    ImageView imageView = helper.getView(R.id.bottomAdvImg);
                    GlideUtils
                            .getInstance()
                            .LoadContextRoundAndCeterCropBitmapDefault(mContext,
                                    UserStorage.getInstance().getBottomAdv().get(0).getPicUrl(),
                                    imageView,
                    ...
                    imageView.setOnClickListener(new View.OnClickListener() {
                        @Override
                        public void onClick(View v) {
                            BottomAdvBean bottomAdvBean = UserStorage.getInstance().getBottomAdv().get(0);
                            ...
                            homeBannerBean.setLinkTypeName(bottomAdvBean.getLinkTypeName());
                            homeBannerBean.setLinkUrl(bottomAdvBean.getLinkUrl());
                            homeBannerBean.setPicUrl(bottomAdvBean.getPicUrl());

                            // EventBusUtil.postEventByEventBus(homeBannerBean, AdvClickEvent.TAG);
                            /**
                             * banner和广告跳转
                             *
                             * @param homeBannerBean
                             */
                            JumpToUtil.getInstance(mContext).jumpToTagActivity(homeBannerBean);
                        }
                    });
                }
                break;
    ...
    private void refreshHistory(BaseViewHolder helper) {
        ...
        if (hRecyclerView == null) {
            ...
            mMineHistoryAdapter = new MineHistoryAdapter(R.layout.item_mine_history_layout,
                    historyBeanList);
...
//        }
        ...
        if (cRecyclerView == null) {
            ...
            mMineCacheAdapter = new MineCacheAdapter(R.layout.item_mine_history_layout,
                    cacheHistoryBeanList);
    ...
    private void refreshLisk(BaseViewHolder helper) {
        ...
        if (lRecyclerView == null) {
            ...
            mMineLikeAdapter = new MineHistoryAdapter(R.layout.item_mine_history_layout,
                    likeHistoryBeanList);
    ...
    private void refreshLayout1(BaseViewHolder helper) {
        //激活兑换码
        View redeemCodeLayout = helper.getView(R.id.redeem_code_layout);
        //等级
...
//        ImageView sexImageView = helper.getView(R.id.mine_sex_view);
        ...
        if (UserStorage.getInstance().getUserType() == UserType.MARK_USER) {
            ...
            if (UserStorage.getInstance().getLevelName() != null) {
                levelTextView.setBackground(null);
                levelTextView.setText(UserStorage.getInstance().getLevelName());
            } else {
//                levelTextView.setText("");
                levelTextView.setBackgroundResource(R.mipmap.login_reg);
            }
...
//            }
        ...
        redeemCodeLayout.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                if (mMineTypeAdapterLisenter != null) {
                    mMineTypeAdapterLisenter.gotoVip();
...
//        }
        ...
        if (UserStorage.getInstance().getViewNum() >= 99999999 || UserStorage.getInstance().getIsVip() == 1) {
            ...
            int userNum =
                    (UserStorage.getInstance().getViewNum() - UserStorage.getInstance().getUsedViewNum());
