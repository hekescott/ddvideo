...
//                    break;
//                case HomeTypeBean.LAYOUT_BOTTOM_ADV://广告
//                    addItemType(HomeTypeBean.LAYOUT_BOTTOM_ADV, R.layout.item_bottom_adv);
//                    break;
                default:
                    break;
            }
        ...
        int advIndex = 0;//等于3的倍数时插入广告
        //插入广告
        if (homeDataBean.getData() != null && homeDataBean.getData().getNewVideoList() != null) {
            advIndex++;
        }
        if (homeDataBean.getData() != null && homeDataBean.getData().getMostVideoList() != null) {
            advIndex++;
        }
        ...
        if (homeDataBean.getData() != null && homeDataBean.getData().getClassifyListCollect() != null) {
            ...
            for (int i = 0; i < homeClassCollectBeanList.size(); i++) {
                advIndex = addAdvIfNeed(advIndex);
    ...
    private int addAdvIfNeed(int advIndex) {
        if (homeDataBean != null
                && homeDataBean.getData() != null
                && homeDataBean.getData().getBottomAdv() != null
                && homeDataBean.getData().getBottomAdv().size() > 0) {
            advIndex++;
            if (advIndex % 3 == 0 && advIndex / 3 > 0) {
                advIndex++;
                BottomAdvBean bottomAdvBean = homeDataBean.getData().getBottomAdv().remove(0);
                addItemType(HomeTypeBean.LAYOUT_BOTTOM_ADV, R.layout.item_bottom_adv);
                HomeTypeBean homeTypeBean = new HomeTypeBean(HomeTypeBean.LAYOUT_BOTTOM_ADV);
                homeTypeBeans.add(homeTypeBean);
                homeTypeBean.setBottomAdvBean(bottomAdvBean);
            }
        }
        return advIndex;
    }
    ...
    @Override
    protected void convert(final BaseViewHolder helper, final HomeTypeBean item) {
        ...
        switch (helper.getItemViewType()) {
                    ...
                    mRecyclerView.addItemDecoration(new RecyclerView.ItemDecoration() {
                        ...
                        public void getItemOffsets(Rect outRect, View view, RecyclerView parent, RecyclerView.State state) {
                            ...
                            if (pos % 2 == 0) {
                                ...
                                outRect.right = ScreenUtil.dip2px(mContext, 0);
                            }
                            outRect.bottom = ScreenUtil.dip2px(mContext, 10);
                        }
                    });
            ...
            break;
            case HomeTypeBean.LAYOUT_BOTTOM_ADV://广告
                ImageView imageView = helper.getView(R.id.bottomAdvImg);
                GlideUtils
                        .getInstance()
                        .LoadContextRoundAndCeterCropBitmapDefault(mContext,
                                item.getBottomAdvBean().getPicUrl(),
                                imageView ,
                ...
                imageView.setOnClickListener(new View.OnClickListener() {
                    ...
                    public void onClick(View view) {
                        if (mHomeTypeAdapterLisenter != null) {
                            BottomAdvBean bottomAdvBean = item.getBottomAdvBean();
                            HomeBannerBean homeBannerBean = new HomeBannerBean();
                            homeBannerBean.setId(bottomAdvBean.getId());
                            homeBannerBean.setTagId(bottomAdvBean.getTagId());
                            homeBannerBean.setTagName(bottomAdvBean.getTagName());
                            homeBannerBean.setLinkType(bottomAdvBean.getLinkType());
                            homeBannerBean.setLinkUrl(bottomAdvBean.getLinkUrl());
                            homeBannerBean.setPicUrl(bottomAdvBean.getPicUrl());
                            mHomeTypeAdapterLisenter.onBottomAdv(homeBannerBean);
                        }
                    }
                });
                break;
...
//                        }
//                    });
//                    HomeManAdapter manAdapter = new HomeManAdapter(R.layout.item_channel_star_layout, homeDataBean.getData().getStarList());
...
//            break;
    ...
    /**
     * 根据recyclerView item的位置获取adv在bottomAdv list中的位置
     * recyclerView item的位置为3、6、9中显示广告
     * @param position
     * @return
     */
    private int getBottomAdvPosition(int position) {
...
//        void onStarMoreMovies();
        ...
        //广告
        void onBottomAdv(HomeBannerBean homeBannerBean);
    }
}
