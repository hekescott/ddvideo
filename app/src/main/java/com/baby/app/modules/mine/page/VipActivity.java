...
//                .setRightText("联系客服")
        ...
        mVipAdapter.setmVipAdapterLisnter(new VipAdapter.VipAdapterLisnter() {
            ...
            public void gotoBuy(VipBean.Data data) {
                ...
                if (withDrawBean.getCurrentCronNum() > 10) {
                    String msg =
                            "您当前钻石余额为" + withDrawBean.getCurrentCronNum() + "，是否使用" + withDrawBean.getMostCron() + "钻石抵扣" + withDrawBean.getMostPrice() + "元";
                    MyDialogUtil.showDimandDialog(VipActivity.this, msg,
                            new DimandDialog.DimandDialogLisenter() {
                        @Override
    ...
    public void refreshPayType(PayBean payBean) {
        MyDialogUtil.showPayDialogDialog(VipActivity.this, payBean.getData(),
                new PayDialog.PayDialogDialogLisenter() {
            @Override
            public void onWxClick(final String payType) {
                new Handler().postDelayed(new Runnable() {
                    ...
                    public void run() {
                        showDialogLoading();
                        if (currentData != null) {
                            mVipPresenter.payRecharge(String.valueOf(currentData.getId()),
                                    payType, String.valueOf(selectType));
                        }
                    }
                }, 100);
...
//        }
    ...
    public void dataCallBack(WithDrawBean withDrawBean) {
        ...
        vip_top_text1.setText(Html.fromHtml("<font size=\"16\" color=\"#ffffff\">当前有</font>" +
                "<font size=\"16\" color=\"#DBB185\">" + withDrawBean.getCurrentCronNum() +
                "</font>" +
